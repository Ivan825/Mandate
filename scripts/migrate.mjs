...
const ca = process.env.DATABASE_SSL_CA ? (await import("node:fs")).readFileSync(process.env.DATABASE_SSL_CA, "utf8") : undefined;
const ssl = mode === "off" ? false : mode === "no-verify" ? { rejectUnauthorized: false } : /localhost|127\.0\.0\.1|@db[:\/]/.test(url) ? undefined : { rejectUnauthorized: true, ca };
