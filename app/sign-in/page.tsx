...
export default async function SignInPage({ searchParams }: { searchParams: Promise<{ next?: string; sent?: string; error?: string }> }) {
  ...
  if (ctx) redirect(next && next.startsWith("/") ? next : "/");
  ...
  return (
    <div style={{ maxWidth: 420, margin: "56px auto" }}>
      ...
      <SignInForm google={google} next={next && next.startsWith("/") ? next : "/"} sent={Boolean(sent)} error={error ?? null} emailDelivery={emailDelivery} />
    </div>
  );
}
