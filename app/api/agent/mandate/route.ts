...
// GET /api/agent/mandate — an agent reads its own limits and what is left,
// so it can plan rather than discover limits by being declined.
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    mandate: m.name, status: m.status, currency: m.currency,
